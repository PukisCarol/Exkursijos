...
def knapsackRecursive(items, index, remaining_weight):
    if index >= len(items) or remaining_weight <= 0:
        return 0, 0, []

    skip_importance, skip_weight, skip_items = knapsackRecursive(items, index + 1, remaining_weight)
...
def saveBestCombination(request, excursion, candidate_items, max_weight_grams):
    best_importance, best_weight, best_items = knapsackRecursive(candidate_items, 0, max_weight_grams)
...
def saveTakenSelection(request, excursion, pupil_items):
    selected_ids = set()
...
def openPupilItemList(request, pk):
    ...
    if request.method == 'POST':
        ...
        if action == 'edit':
            pupil_items = list(PupilBackpackItem.objects.filter(
                pupil=request.user,
                excursion=excursion,
            ).select_related('item').order_by('item__name'))

            saveTakenSelection(request, excursion, pupil_items)
